...
(define updated-layout-tests
  (test-suite "Updated Layout Tests"
    ...
    (test-case "E2 to E1 layout"
      ...
      (check-equal? (updated-layout layout-many-E2) layout-many-E1)))
...
(define convert-tests
  ...
  (test-suite "convert Tests"
    (test-case "convert some positions to E2"
      (let ([converted-layout
             (convert layout-all-W (list (make-cor 1 1) (make-cor 0 0)) 'E2)])
        (check-equal? (get-symbol converted-layout (make-cor 1 1)) 'E2)
        (check-equal? (get-symbol converted-layout (make-cor 0 0)) 'E2)
...
(define boom-tests
  (test-suite "Boom Function Tests"
    ;; Test 1: No bombs, players exist but unaffected
    ...
    ;; Test 2: Single bomb, players out of range
    (test-case "Single bomb, players out of range: Layout updates correctly"
      (let* (
             [layout-all-W
              (vector
               (vector 'W 'W 'W)
    ...
    ;; Test 3: Single bomb with player1 in range
    ...
    ;; Test 4: Both players in explosion range
    ...
    ;; Test 5: Multiple bombs, chain reaction, players unaffected
    (test-case "Multiple bombs with chain reaction, no players affected"
      (let* (
             [layout-all-W
              (vector
               (vector 'W 'W 'W)
  ...
  ;; Test 6: Single bomb, player2 in range
  ...
  ;; Test 7: Multiple bombs, chain reaction with player1 in range
  (test-case "Chain reaction with player1 in range"
    (let* ([layout-all-W
              (vector
               (vector 'W 'W 'W)
               (vector 'W 'W 'W)
  ...
  ;; Test 8: No chain reaction, players unaffected
  (test-case "No chain reaction: Layout and players remain correct"
    (let* ([layout-all-W
              (vector
               (vector 'W 'W 'W)
               (vector 'W 'W 'W)
  ...
  ;; Test 9: Bomb countdown > 0, no explosion
  ...
  ;; Test 10: Complex chain reaction with all players affected
  (test-case "both players in range"
    (let* ([layout-all-W
              (vector
               (vector 'W 'W 'W)
               (vector 'W 'W 'W)
      ...
      (let (
        ...
        (check-equal? (timehandler example-state)
                      (make-gamestate test-layout
                                      ...
                                      3
                                      #f))))
    (test-case "Roundtimer triggers maximum increment"
      (let (
            [example-state
             (make-gamestate test-layout
                             '()
